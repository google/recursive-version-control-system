...
func verifyFilesMatch(t *testing.T, file1, file2 string) {
	// Validate that the cloned file matches the original...
	if originalBytes, err := os.ReadFile(file1); err != nil {
		t.Errorf("failure reading the first file contents: %v", err)
	} else if clonedBytes, err := os.ReadFile(file2); err != nil {
		t.Errorf("failure reading the second file contents: %v", err)
...
func TestCheckoutOverwriteRegularFile(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "archive")
	s := &storage.LocalFiles{ArchiveDir: archive}
	...
	clone := filepath.Join(dir, "clone.txt")
	if err := os.WriteFile(clone, []byte("Conflicting hello, World!"), 0777); err != nil {
		t.Fatalf("failure creating the example file to overwrite: %v", err)
	}
	clonePath := snapshot.Path(clone)
	if err := Checkout(context.Background(), s, h1, clonePath); err != nil {
		t.Fatalf("failure checking out the file snapshot %q: %v", h1, err)
...
func TestCheckoutOverwriteSymlink(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "archive")
	s := &storage.LocalFiles{ArchiveDir: archive}
...
func TestCheckoutExcludedDir(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "archive")
	s := &storage.LocalFiles{ArchiveDir: archive}
	...
	if err := Checkout(context.Background(), s, h1, dirPath); err != nil {
...
func TestCheckoutDir(t *testing.T) {
	...
	if err := Checkout(context.Background(), s, h1, cloneDirPath); err != nil {
		t.Fatalf("failure checking out the directory snapshot %q: %v", h1, err)
	}
...
func TestCheckoutOverwriteDir(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "archive")
	s := &storage.LocalFiles{ArchiveDir: archive}
	...
	if err := os.Mkdir(cloneDir, 0700); err != nil {
		t.Fatalf("failure creating the clone directory for the test: %v", err)
	}
	cloneDirPath := snapshot.Path(cloneDir)
	cloneFile1 := filepath.Join(cloneDir, "example1.txt")
	cloneFile2 := filepath.Join(cloneDir, "example2.txt")
	if err := os.WriteFile(cloneFile1, []byte("Hello, World 1!"), 0700); err != nil {
		t.Fatalf("failure creating the example file 1 to overwrite: %v", err)
	}
	if err := os.WriteFile(cloneFile2, []byte("Hello, World 2... but different from the original!"), 0700); err != nil {
		t.Fatalf("failure creating the example file 2 to overwrite: %v", err)
	}
...
func TestCheckoutOverwriteTextFileWithDir(t *testing.T) {
	dir := t.TempDir()
	archive := filepath.Join(dir, "archive")
	s := &storage.LocalFiles{ArchiveDir: archive}
	...
	cloneDir := filepath.Join(dir, "clone-dir")
	if err := os.WriteFile(cloneDir, []byte("Text file that will be overwritten!"), 0700); err != nil {
		t.Fatalf("failure creating the example file to overwrite: %v", err)
	}
	cloneDirPath := snapshot.Path(cloneDir)
	if err := Checkout(context.Background(), s, h1, cloneDirPath); err != nil {
